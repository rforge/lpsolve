...
The~\texttt{>} shown before each R command is the R prompt. Only the text after~\texttt{>} should be entered.
...
Note that there are some commands that return an answer. For the \emph{get} accessor functions (the functions with names beginning with the word \textit{get}) the output should be clear. For other functions (e.g., solve), the interpretation of the returned value is described in the documentation. Since solve is generic in R, use the command

<<eval=FALSE>>=
help(solve.lpExtPtr)
@

to view its documentation. The \emph{set} accessor functions (the functions with names beginning with the word \textit{set}) do not have return values.  If the return without error, you may assume that they completed successfully.
