...
static void add_int_var(char *name, short int_decl)
{
  ...
  if((hp = findhash(name, Hash_tab)) == NULL) {
    ...
    if(int_decl == 2) {
      if(coldata[hp->index].lowbo != -DEF_INFINITE * (LPSREAL) 10.0) {
        char buf[256];
...
int set_sec_threshold(char *name, LPSREAL threshold)
{
  hashelem *hp;
...
static int inccoldata(void)
{
  ...
  if(coldata != NULL) {
    ...
    coldata[Columns].lowbo = (LPSREAL) -DEF_INFINITE * (LPSREAL) 10.0; /* temporary. If still this value then 0 will be taken */
 ...
 * if type = true then
 */
int rhs_store(LPSREAL value, int HadConstraint, int HadVar, int Had_lineair_sum)
{
 ...
 */
int var_store(char *var, LPSREAL value, int HadConstraint, int HadVar, int Had_lineair_sum)
{
  int row;
...
static int readinput(lprec *lp)
{
  ...
  if((lp != NULL) &&
     ((MALLOC(row, 1 + Rows, LPSREAL) == NULL) || (MALLOC(rowno, 1 + Rows, int) == NULL))) {
    FREE(SOSrowdata);
    FREE(negateAndSOS);
    FREE(row);
    FREE(rowno);
...
lprec *yacc_read(lprec *lp, int verbose, char *lp_name, int *_lineno, int (*parse) (void), void (*delete_allocated_memory) (void))
{
  ...
  if((stat != 0) || (CALLOC(relat, Rows + 1, short) != NULL)) {
	...
	if(Rows) {
	  ...
	  MALLOCCPY(orig_upbo, lp->orig_upbo, 1 + Rows, LPSREAL);
