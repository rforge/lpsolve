add.constraint <- function(lprec, xt, type = c("<=", "=", ">="), b, indices = NULL)
{
  n <- dim(lprec)[2]
  ...
  invisible(.Call("RlpSolve_add_constraintex", lprec, as.double(xt), as.integer(indices),
                  as.integer(type), as.double(b)))
}
